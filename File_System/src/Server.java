	...
	public Server(String args[]) throws IOException {
		...
		try {
			...
			if ((server_no) == 1) 
			{
				...
				s[5] = ss[5].accept();
				oos[5] = new ObjectOutputStream(s[5].getOutputStream());
				ois[5] = new ObjectInputStream(s[5].getInputStream());
				...
				s[6] = ss[6].accept();
				oos[6] = new ObjectOutputStream(s[6].getOutputStream());
				ois[6] = new ObjectInputStream(s[6].getInputStream());
			...
			if ((server_no) == 2) {
				...
				oos[1] = new ObjectOutputStream(s[1].getOutputStream());
				ois[1] = new ObjectInputStream(s[1].getInputStream());
			...
			if ((server_no) == 3) {
				...
				oos[2] = new ObjectOutputStream(s[2].getOutputStream());
				ois[2] = new ObjectInputStream(s[2].getInputStream());
			...
			if ((server_no) == 4) {
				...
				oos[3] = new ObjectOutputStream(s[3].getOutputStream());
				ois[3] = new ObjectInputStream(s[3].getInputStream());
			...
			if ((server_no) == 5) {
				...
				oos[4] = new ObjectOutputStream(s[4].getOutputStream());
				ois[4] = new ObjectInputStream(s[4].getInputStream());
			...
			try {
				...
				while (messagefromclient != "over") {
					...
					if (objinput.available() > 0) {
						...
						System.out.println("Message from the client " + object.getSenderID()+"of type "+messagetype);
						...
						if (messagetype.equals("READFILERESPONSE")) {
							...
							String filenameatmylocation = filename.split("_")[0]+"_"+filename.split("_")[1]+"_"+filename.split("_")[2]+"_"+server_no;
							try
